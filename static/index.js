  ...
  scan = async () => {
    try {
      ...
      refreshIntervalId = setInterval(async () => {
        if (finished) {
          clearInterval(refreshIntervalId)
          await axios.delete('http://localhost/api/token/' + token)
          unLoadTrigger()
        } else {
          const getResponse = await axios.get('http://localhost/api/token/' + token)
          console.log(getResponse.data.results)
          getResponse.data.results.map(async ip => {
            console.log(ip)
            if (ip.ports.length >= 1) {
              ip.ports.map(p => {
                console.log(ip.ip + '-----' + p.port)
                if (!responseIps.includes(ip.ip)) {
                  responseIps.push(ip.ip)
                  responsePorts.push(p.port)
                  addElement(ip)
                }
              })
            }
          })
          console.log(responseIps)
          console.log(responsePorts)
          if (getResponse.data.results.length >= hosts.length) {
            finished = true
          }
